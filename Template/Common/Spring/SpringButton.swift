...
open class SpringButton: UIButton, Springable, Gradientable {
    ...
    var impactStyle = TapticEngine.Impact.ImpactStyle.medium

    @IBInspectable var applyGradient: Bool = true
    ...
    private func setupGradient() {
        if let imageView = imageView, let _ = imageView.image {
            addGradientOn(view: gradientView, at: 0)
            gradientView.mask = imageView
            addSubview(gradientView)
            gradientView.constraintsToParent(view: self)
        } else if let bgImageView = self.subviews.first as? UIImageView,
                  let _ = bgImageView.image {
            addGradientOn(view: gradientView, at: 0)
            gradientView.mask = bgImageView
            addSubview(gradientView)
            gradientView.constraintsToParent(view: self)
        }
    ...
    @objc private func setupGradientColor() {
        let theme = Theme.current
        updateGradientColors(colors: theme.gradientColors)
    }
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if isEnabled {
            TapticEngine.impact.feedback(impactStyle)
            animate(name:"morph")
        }
    }
}
